...
def main():
    ...
    if args.dataset != 'imagenet':
        model = nn.Sequential(normalize, eval(args.arch)(num_classes=data.cls_dict[args.dataset])).to(device)
    else:
        model = nn.Sequential(normalize, eval('torchvision.models.' + args.arch + "()")).to(device)
    ...
    if args.dataset != 'imagenet':
        ...
        delta = (torch.rand([n_ex] + data.shapes_dict[args.dataset], dtype=torch.float32, device=device) * 2 - 1) * args.epsilon
    else:
        train_loader, test_loader, n_ex = data.load_data_imagenet(args.dataset, args.batch_size, args.num_workers)
        delta = (torch.rand([n_ex] + data.shapes_dict[args.dataset], dtype=torch.float16, device=device) * 2 - 1) * args.epsilon
    ...
    gdnorm = torch.zeros((n_ex), dtype=torch.float32, device=device)
